...
const TABLE = 'prismCentral'

class PrismCentral {
  constructor(pc) {
    this.id = pc.id
    this.displayName = pc.displayName
    this.hostnameOrIP = pc.hostnameOrIP

    this.credentials = typeof pc.credentials == 'string' ? crypto.decrypt(pc.credentials) : pc.credentials
    this.default = Boolean(pc.default) || Boolean(pc.siteDefault)
  ...
  static fromDB(record) {
    let pc = new PrismCentral(record)
    pc._record = record
