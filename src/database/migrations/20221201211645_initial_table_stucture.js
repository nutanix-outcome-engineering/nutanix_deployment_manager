...
exports.up = async (knex) => {
  await knex.schema.createTable('ingestData', table => {
  ...
  await knex.schema.createTable('switch', table => {
    table.increments('id').primary()
    table.enum('type', ['ipmmi', 'tor'])
    table.string('ip', 15)
    table.string('name', 50)
  ...
  await knex.schema.createTable('hypervisor', table => {
    table.increments('id').primary()
    table.string('name', 50)
  ...
  await knex.schema.createTable('aos', table => {
    table.increments('id').primary()
    table.string('name', 50)
    table.string('version', 50)
    table.string('filename', 50)
  ...
  await knex.schema.createTable('cluster', table => {
    table.increments('id').primary()
    table.string('name', 50)
  ...
  await knex.schema.createTable('ntnxConfig', table => {
    table.increments('id').primary()
    table.string('type', 50)
    table.json('config')
  })
...
exports.down = async (knex) => {
  await knex.schema.dropTableIfExists('switch')
  await knex.schema.dropTableIfExists('node')
  await knex.schema.dropTableIfExists('cluster')
  await knex.schema.dropTableIfExists('hypervisor')
  await knex.schema.dropTableIfExists('aos')
  await knex.schema.dropTableIfExists('rack')
  await knex.schema.dropTableIfExists('site')
  await knex.schema.dropTableIfExists('ntnxConfig')
  await knex.schema.dropTableIfExists('ingestData')
