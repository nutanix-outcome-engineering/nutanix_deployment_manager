...
class PostUploadHypervisorTask {
  constructor() {
  }
  ...
  static async process(job, token) {
    const uploadResults = Object.values(await job.getChildrenValues())[0]
    const hypervisor = await Hypervisor.getById(uploadResults.hypervisorUUID)
    await hypervisor.moveToFinalDestination(uploadResults.filepath, uploadResults.exportBasePath)
    hypervisor.transferStatus = 'uploaded'
    hypervisor.filename = `${hypervisor.uuid}.iso`
    await hypervisor.update()

    const taskFlow = await TaskFlowFactory.getByID(job.data.taskFlowId)
    taskFlow.complete({name: job.data.taskType}, hypervisor.toJSON())
    await taskFlow.update()
...
module.exports = {
  UploadAOSTask,
  PostUploadAOSTask,
  UploadHypervisorTask,
  PostUploadHypervisorTask
}
